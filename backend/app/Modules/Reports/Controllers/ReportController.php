...
class ReportController extends Controller
{
    ...
    public function financialSummaryByServiceCodeReport(): void
    {
        $request = new Request();
        $filters = $request->only(['facility_id', 'date_from', 'date_to', 'provider_id']);
        $data = $this->reportService->getFinancialSummaryByServiceCodeReport($filters);
    ...
    public function indigentPatientsReport(): void
    {
        $request = new Request();
        $filters = $request->only(['date_from', 'date_to']);
        $data = $this->reportService->getIndigentPatientsReport($filters);
    ...
    {
        $request = new Request();
        $filters = $request->only(['facility_id', 'date_from', 'date_to', 'provider_id', 'date_type', 'procedure_code', 'diagnosis_code']);
        $data = $this->reportService->getCashReceiptsByProviderReport($filters);
        $this->success($data, 'Cash receipts by provider report retrieved successfully.');
    }

    public function frontOfficeReceiptsReport(): void
    {
        $request = new Request();
        $filters = $request->only(['facility_id', 'date_from', 'date_to', 'provider_id']);
