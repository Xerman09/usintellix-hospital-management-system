...
namespace App\Modules\ProviderCategories\Services;

use App\Core\Database;
use App\Modules\ProviderCategories\Models\ProviderCategory;
use PDO;
use Throwable;
...
{
    /**
     * List all active (non-deleted) provider categories.
     */
    ...
    public function register(array $data, int $createdBy): array
    {
        ...
        try {
            $categoryId = (new ProviderCategory())->create([
                'name'        => $data['name'],
            ...
            if (!$categoryId) {
                throw new \RuntimeException('Failed to create provider category record.');
            }
    ...
    {
        $category = (new ProviderCategory())->where('id', $id)->first();
        ...
        $updated = (new ProviderCategory())->update([
            'name'        => $data['name'],
            'description' => $data['description'] ?? null,
    ...
    /**
     * Validate provider category input.
     */
    private function validate(array $data, ?int $ignoreId = null): array
    {
        ...
        $existing = (new ProviderCategory())->where('name', $data['name'])->first();

        if ($existing && $existing['deleted_at'] === null && (int) $existing['id'] !== (int) $ignoreId) {
            $errors['name'] = 'A provider category with this name already exists.';
        }
