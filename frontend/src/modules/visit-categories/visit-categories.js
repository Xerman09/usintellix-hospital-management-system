...
import {
    ...
    updateVisitCategory,
    deleteVisitCategory
} from "./visit-categories.service.js";
...
export async function initVisitCategories()
{
    const user = getUser();
    ...
    const modalOverlay = document.getElementById("visitCategoryModalOverlay");
    const modalTitle = document.getElementById("visitCategoryModalTitle");
    const saveBtn = document.getElementById("saveVisitCategoryBtn");
    ...
    const openModal = (category) => {
        ...
        if (category) {
            modalTitle.textContent = "Edit Visit Category";
            saveBtn.textContent = "Save Changes";
            idInput.value = category.id;
            document.getElementById("name").value = category.name ?? "";
            document.getElementById("description").value = category.description ?? "";
        } else {
            modalTitle.textContent = "Add Visit Category";
            saveBtn.textContent = "Add Visit Category";
            idInput.value = "";
            form.reset();
        }
    ...
    document.getElementById("openAddVisitCategoryModal").addEventListener("click", () => openModal(null));
    document.getElementById("closeVisitCategoryModal").addEventListener("click", closeModal);
    document.getElementById("cancelVisitCategory").addEventListener("click", closeModal);
    ...
    form.addEventListener("submit", async (event) => {
        ...
        const result = editingId
            ? await updateVisitCategory(editingId, data)
            : await createVisitCategory(data);

        if (!result.success) {
            showAlert(result.message || "Failed to save visit category.", "error");
...
async function loadVisitCategories(openModal)
{
...
{
    const tbody = document.getElementById("visitCategoriesTableBody");
    const countText = document.getElementById("visitCategoryCountText");
    ...
    tbody.querySelectorAll("[data-id]").forEach((btn) => {
        btn.addEventListener("click", async () => {
            if (!confirm("Remove this visit category?")) {
                return;
            }
...
{
    const heading = noneAtAll ? "No visit categories yet" : "No matching categories";
    const message = noneAtAll
        ? "Create your first category to start classifying patient visits."
